...
#Plotting, 
png("plot4.png",height=480,width=480)#Setting the height and width given.
par(mfrow = c(2, 2))#Setting/making grid.
with(newdata,{#Placing them here as all have to be on one page.
    plot(newdata$Global_active_power~newdata$DateAndTime,type="l",ylab="Global Active Power",xlab="")
    plot(newdata$Voltage~newdata$DateAndTime,type="l",ylab="Voltage",xlab="datetime")
    ...
    plot(newdata$Global_reactive_power~newdata$DateAndTime,type="l",ylab="Global_reactive_power",xlab="datetime")})
dev.off()
